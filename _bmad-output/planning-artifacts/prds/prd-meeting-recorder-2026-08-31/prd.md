...
- **Settled in increment 11, and it holds — on the probe.** With the two device starts made adjacent (FR-105), the measured offset is **+14, +15, +15, +16, +18 ms** across five captures through the installed shipping build, against a baseline of **+40, +41, +62 ms**. The serialisation term is **+0.0 to +0.1 ms**, so what remains is the difference between the two devices' first-callback latencies, which capture does not control. ±100 ms therefore stands as written and is no longer an assertion. **What is not yet confirmed is a real meeting**: the +1,006 ms reading came from a 42-minute Session on loudspeakers, and the three other instrumented recordings read +21, +53 and +60 ms through the *unfixed* path — so the outlier's condition has not been reproduced either way, and the instrument is in place to answer it on the next such Session without anyone remembering to look.
