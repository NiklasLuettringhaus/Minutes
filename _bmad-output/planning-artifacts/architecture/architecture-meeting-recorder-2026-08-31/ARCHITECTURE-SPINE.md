...
- **Amended 2026-09-02, by the owner's decision, and the rule above is now weaker than it was written.** Homebrew distribution is dropped and the Developer ID is not being bought yet. The shipped install path is a GitHub Release archive plus `xattr -dr com.apple.quarantine`, which **is** the bypass this AD was written to forbid. It is recorded here rather than quietly done: the cost is that the first install teaches a bypass, and that every update still revokes microphone and system-audio consent, which FR-73 exists to prevent and which this path does not deliver. Two things keep it honest — the installer states what it is doing while it does it, and building from source remains documented as the path that bypasses nothing, because a locally compiled app is never quarantined. The rule's original form is the target, not a description of today; revisit when the app has more than one or two users, at which point the recurring consent cost exceeds the certificate's.
